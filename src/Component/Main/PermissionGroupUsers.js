...
export const PermissionGroupUsers = ({
    UserInfo = {},
    ChangePermission = () => { }
}) => {
    ...
    useEffect(() => {
        Shop_spGroupUserPermission_Get()
    }, [UserInfo]);
    ...
    const handleSelectAll = () => {
        ...
        const updatedMenus = Menus.map((item) => {
            return {
                ...item,
                IsGroupPermission: selectAll ? 0 : 1, // Đặt trạng thái hoạt động của tất cả các switch thành 0 hoặc 1 tùy thuộc vào giá trị của selectAll
            };
        });
    ...
    const handleChange = (index) => {
        return () => {
            ...
            updatedMenus[index].IsGroupPermission = updatedMenus[index].IsGroupPermission === 1 ? 0 : 1;
            setMenus(updatedMenus);
        };
    };

    //#region List
    const Shop_spGroupUserPermission_Get = async () => {
        try {
            const params = {
                Json: JSON.stringify({
                    UserId: UserInfo.ID,
                }),
                func: "Shop_spGroupUserPermission_Get",
            };
    ...
    //#region Save
    const Shop_spUserGroupPermission_Save = async () => {
         
        try {
            setDisable3(false);
            let GroupUserPermissionTmp = [];
            if (Array.isArray(Menus)) {
                GroupUserPermissionTmp = Menus
                    .filter(e => e.IsGroupPermission === 1)
                    .map(e => ({
                        UserId: UserInfo?.ID,
                        MenuId: e.Id,
                    }));
            ...
            const params = {
                Json: JSON.stringify({
                    Clear: GroupUserPermissionTmp.length,
                    UserInfo: UserInfo?.ID,
                    UserGroupPermission: GroupUserPermissionTmp,
                }),
                func: "Shop_spUserGroupPermission_Save",
            };
            setDisable3(true);
            const result = await mainAction.API_spCallServer(params, dispatch);
                            ...
                            <tr key={index}>
                                ...
                                <td>
                                    <input
                                        ...
                                        checked={item.IsGroupPermission === 1 ? true : false} // Đặt trạng thái hoạt động của switch dựa trên giá trị 'IsGroupPermission'
                                ...
                                <td>
                                    <div style={{ textAlign: 'center' }}>
                                        {item.IsGroupPermission === 1 && <i className="fa-solid fa-circle-check checkok"></i>}
                                    </div>
                                </td>
                            </tr>
                        );
            ...
            <div className="row">
                <div className="col-md-12 text-center mt-3 mb-3">
                    <button
                        ...
                        className="btn btn-sm btn-success"
                        disabled={!Disable3}
                        onClick={(e) => Shop_spUserGroupPermission_Save()}
                    >
                        <span>
