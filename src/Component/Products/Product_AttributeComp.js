...
import { SelectAttribute } from "../../Common";

export const Product_AttributeComp = ({
  ListData = [],
  onData = () => { },
  readOnly = false
}) => {
  ...
  return (
    <>
      <>
        <label className="no-absolute">Thuộc tính sản phẩm</label>
            ...
            {Data?.filter((p) => p.IsDelete === false).map((item, index) => {
              return (
                <tr key={index}>
                  ...
                  <td>
                    <SelectAttribute
                      Data={[{ value: item.AttributeId }]}
                      onSelected={(e) => {
                        updateTableData(index, "AttributeId", e.value);
                        setFlag(!Flag)
                      }}
                      isDisabled={readOnly}
                    />
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) =>
                        updateTableData(index, "Description", e.target.value)
                      }
                      type="text"
